...
EndType = enum(EndTypeCustom=0, EndTypeSuctionCup=1, EndTypeGripper=2, EndTypeLaser=3, EndTypePen=4, EndTypeMax=5)
...
OutputPin = enum(
    SIGNALS_O1=1, SIGNALS_O2=2, SIGNALS_O3=3, SIGNALS_O4=4, SIGNALS_O5=5, SIGNALS_O6=6, SIGNALS_O7=7, SIGNALS_O8=8
)
...
GPIOType = enum(
    GPIOTypeDummy=0, GPIOTypeDO=1, GPIOTypePWM=2, GPIOTypeDI=3, GPIOTypeADC=4, GPIOTypeDIPU=5, GPIOTypeDIPD=6
)
...
DobotConnect = enum(DobotConnect_NoError=0, DobotConnect_NotFound=1, DobotConnect_Occupied=2)
    ...
    try:
        if masterDevType == DevType.Conntroller:
            if connectInfo.slaveDevInfo1.type == 0 and connectInfo.slaveDevInfo2.type == 0:
                ...
                fwName = (
                    str(connectInfo.slaveDevInfo1.firmwareName, encoding="utf-8").strip(b"\x00".decode())
                    if connectInfo.slaveDevInfo1.type != DevType.Idle
                    else str(connectInfo.slaveDevInfo2.firmwareName, encoding="utf-8").strip(b"\x00".decode())
                )
                fwVer = (
                    str(connectInfo.slaveDevInfo1.firwareVersion, encoding="utf-8").strip(b"\x00".decode())
                    if connectInfo.slaveDevInfo1.type != DevType.Idle
                    else str(connectInfo.slaveDevInfo2.firwareVersion, encoding="utf-8").strip(b"\x00".decode())
                )
                slaveDevType = (
...
def GetDeviceVersion(api):
    ...
    if masterDevType == DevType.MagicianLite:
        while True:
...
def SetEndEffectorParams(api, xBias, yBias, zBias, isQueued=0):
    ...
    while True:
        result = api.SetEndEffectorParams(
            c_int(masterId), c_int(slaveId), byref(param), isQueued, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEndEffectorLaser(api, enableCtrl, on, isQueued=0):
    ...
    while True:
        result = api.SetEndEffectorLaser(
            c_int(masterId), c_int(slaveId), enableCtrl, on, isQueued, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetJOGJointParams(
    ...
    while True:
        result = api.SetJOGJointParams(
            c_int(masterId), c_int(slaveId), byref(jogParam), isQueued, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
    ...
    if cmd == 0:
        ...
        while True:
            result = api.SetJOGCmd(
                c_int(masterId), c_int(tempSlaveId), byref(cmdParam), isQueued, byref(queuedCmdIndex)
            )
            if result != DobotCommunicate.DobotCommunicate_NoError:
                dSleep(5)
                continue
...
def SetPTPCoordinateParams(api, xyzVelocity, xyzAcceleration, rVelocity, rAcceleration, isQueued=0):
    ...
    while True:
        result = api.SetPTPCoordinateParams(
            c_int(masterId), c_int(slaveId), byref(pbParam), isQueued, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetIOMultiplexing(api, address, multiplex, isQueued=0):
    ...
    while True:
        result = api.SetIOMultiplexing(
            c_int(masterId), c_int(tempSlaveId), byref(param), isQueued, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def GetAngleSensorStaticError(api):
    ...
    while True:
        result = api.GetAngleSensorStaticError(
            c_int(masterId), c_int(slaveId), byref(rearArmAngleError), byref(frontArmAngleError)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def GetAngleSensorCoef(api):
    ...
    while True:
        result = api.GetAngleSensorCoef(
            c_int(masterId), c_int(slaveId), byref(rearArmAngleCoef), byref(frontArmAngleCoef)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetColorSensor(api, isEnable, colorPort, version=0):
    ...
    while True:
        result = api.SetColorSensor(
            c_int(masterId), c_int(tempSlaveId), enable, port, version, 1, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def GetUpgradeFWReadyCmd(api, fwSize, md5):
    ...
    while True:
        result = api.GetUpgradeFWReadyCmd(
            c_int(masterId), c_int(tempSlaveId), byref(upgradeFWReadyCmd), byref(isUpgrade)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetInfraredSensorExt(api, isEnable, infraredPort, version=0, isQueued=0):
    ...
    while True:
        result = api.SetInfraredSensor(
            c_int(masterId), c_int(-1), enable, port, version, isQueued, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetLostStepEnableAndParamsCmd(api, enable, threshlod, isQueued=0):
    ...
    while True:
        result = api.SetLostStepEnableAndParamsCmd(
            c_int(masterId), c_int(slaveId), c_uint8(enable), c_float(threshlod), isQueued, byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetEndEffectorType(api, endType=0, isQueued=0):
    ...
    while True:
        result = api.SetEndEffectorType(
            c_int(masterId), c_int(slaveId), isQueued, c_uint8(endType), byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetServoAngle(api, servoId, angle, isQueued=0):
    ...
    while True:
        result = api.SetServoAngle(
            c_int(masterId), c_int(-1), isQueued, c_uint8(servoId), c_float(angle), byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetArmSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    ...
    while True:
        result = api.SetArmSpeedRatio(
            c_int(masterId), c_int(slaveId), isQueued, c_uint8(paramsMode), c_uint8(speedRatio), byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
...
def SetLSpeedRatio(api, paramsMode, speedRatio, isQueued=0):
    ...
    while True:
        result = api.SetLSpeedRatio(
            c_int(masterId), c_int(-1), isQueued, c_uint8(paramsMode), c_uint8(speedRatio), byref(queuedCmdIndex)
        )
        if result != DobotCommunicate.DobotCommunicate_NoError:
            dSleep(5)
            continue
