...
class ImageValidator:
    ...
    def validate_pred_score(pred_score: torch.Tensor | float) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Validated prediction score as a float32 tensor.

        Raises:
            TypeError: If the input is neither a float nor a torch.Tensor.
...
class ImageBatchValidator:
    ...
    def validate_anomaly_map(anomaly_map: torch.Tensor | np.ndarray | None) -> Mask | None:
        ...
        Args:
            anomaly_map (torch.Tensor | None): Input batch of anomaly maps.
    ...
    @staticmethod
    def validate_pred_score(pred_score: torch.Tensor | list[float]) -> torch.Tensor:
        """Validate the prediction scores for a batch.

        Args:
            pred_score (torch.Tensor | list[float]): Input prediction scores.

        Returns:
            torch.Tensor: Validated prediction scores.

        Raises:
            TypeError: If the input is neither a torch.Tensor nor a list of floats.
        ...
        if isinstance(pred_score, list):
            pred_score = torch.tensor(pred_score)
        if not isinstance(pred_score, torch.Tensor):
            msg = f"Prediction scores must be a torch.Tensor or list[float], got {type(pred_score)}."
            raise TypeError(msg)
