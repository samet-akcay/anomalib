...
def overlay_images(base: Image.Image, overlays: Image.Image | list[Image.Image], alpha: float = 0.5) -> Image.Image:
    ...
    for overlay in overlays:
        base = overlay_image(base, overlay, alpha=alpha)
...
def get_field_kwargs(field: str) -> dict[str, Any]:
    ...
    if func is None or not callable(func):
        return f"'{field}' is not a valid function in the current module."
